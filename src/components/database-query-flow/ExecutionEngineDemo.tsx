...
export function ExecutionEngineDemo() {
	...
	// Helper to determine block state during Index Scan
	const getIndexBlockState = (index: number) => {
		if (activeScan !== "index") return "idle";
		if (currentStep < 2) return "idle";
		if (index === TARGET_BLOCK) {
			if (currentStep === 2) return "reading";
			if (currentStep >= 3) return "found";
		}
		return "idle";
	};
				...
				<div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 flex flex-col md:flex-row gap-6 items-center justify-between">
					<div>
						...
						<p className="text-sm text-zinc-400 mt-1 max-w-lg">
							The Executor implements the Volcano (Iterator) Model. A parent
							node calls <code>Next()</code> on its children. Watch how{" "}
					...
					<div className="flex items-center gap-3">
						...
						<button
							...
							disabled={isPlaying}
							className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed border border-emerald-500 rounded-lg text-sm font-medium transition-colors text-white"
						>
							▶ Play Index Scan
						</button>
						<button
					...
					<div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex flex-col">
						...
						<div className="flex-1 grid grid-cols-4 gap-3 content-start">
							{Array.from({ length: TOTAL_BLOCKS }).map((_, i) => {
								...
								const isActive =
									seqState === "reading" || idxState === "reading";
								...
								return (
									...
									>
										<span
											className={`font-mono text-sm font-bold ${labelClass}`}
										>
											Page {i}
										</span>
