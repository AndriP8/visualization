...
export function CacheStampedeDemo() {
	...
	return (
		<DemoSection
			title="Demo 4: Cache Stampede (Thundering Herd)"
			description="When a heavily-requested cache key expires, hundreds of simultaneous requests all miss the cache at once and hammer the database simultaneously. Explore mitigations: Probabilistic Early Expiration (XFetch) and Single-Flight Locking."
		>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									...
									>
										...
										{isLocked && phase === "queue" && (
											<motion.div
												...
												className="absolute w-3 h-3 rounded-full flex items-center justify-center -translate-x-1/2"
												style={{ backgroundColor: dot.color }}
											>
												{isLocked && (
													<span className="text-[8px] absolute">🔒</span>
												)}
											</motion.div>
										)}
									</motion.div>
					...
					<div className="space-y-4">
							...
							<div className="space-y-3 text-sm text-zinc-400">
								...
								<ul className="space-y-2">
									<li className="flex gap-2">
										<span className="text-rose-400">1.</span>N concurrent
						...
						{selected === "early-expiry" && (
							<div className="space-y-3 text-sm text-zinc-400">
								...
								<p className="text-xs">
									...
									introduces probabilistic spread — some requests refresh early
									at random times, preventing a thundering herd at the exact
									expiry moment. Based on the XFetch paper (Vattani et al.,
									2015).
								</p>
							</div>
						)}
						...
						{selected === "single-flight" && (
							<div className="space-y-3 text-sm text-zinc-400">
								<p>
									Deduplicate in-flight requests with a pending promise map:
								</p>
								<ShikiCode
									language="javascript"
									code={
								...
								/>
								<p className="text-xs">
									All concurrent requests for the same key share one Promise.
									Only the first caller hits the DB — others wait and receive
									the same result.
								</p>
							</div>
						)}
