...
const MATRIX: Record<MatrixMode, Record<string, Record<string, CellState>>> = {
	...
	postgres: {
		...
		rr: {
			...
			phantom: "not_in_pg",
			serialization: "possible",
		}, // PG RR also prevents phantom reads!
		sz: {
			dirty: "not_possible",
			nonrepeatable: "not_possible",
			phantom: "not_possible",
...
const EXPLANATIONS: Record<MatrixMode, Record<string, string>> = {
	ansi: {
		ru: "Allows everything. Transactions can read dirty (uncommitted) data from other transactions.",
		rc: "Default in most SQL systems. Prevents Dirty Reads, but a query run twice in the same transaction may see different values if another tx commits in between.",
		rr: "Ensures that if you read a row, it will have the exact same values if you read it again later. However, new rows ('phantoms') might appear if added by another tx.",
		sz: "Strict sequential execution guarantee. Completely prevents all theoretical anomalies, but incurs the highest locking/serialization failure overhead.",
	},
	postgres: {
		ru: "In PostgreSQL, Read Uncommitted is automatically mapped to Read Committed. Dirty reads are physically impossible under PG's MVCC architecture.",
		rc: "PostgreSQL's default. Each statement sees a snapshot of committed data taken at the start of that individual query statement.",
		rr: "PostgreSQL's Repeatable Read uses MVCC snapshots taken at the start of the entire TRANSACTION, thus it naturally prevents Phantom Reads as well!",
		sz: "Uses SSI (Serializable Snapshot Isolation). Monitors read/write dependencies across concurrent transactions and automatically aborts with a serialization failure if a dangerous anomaly cycle is detected.",
	},
};
			...
			<div className="flex flex-col gap-6">
						...
						>
							{mode === "ansi" && (
								<motion.div
									...
									className="absolute inset-0 bg-emerald-600 rounded drop-shadow-md"
									initial={false}
				...
				<div className="flex flex-col lg:flex-row gap-6">
					...
					<div className="w-full lg:w-1/3 bg-zinc-800/40 rounded-lg p-5 border border-zinc-700 relative overflow-hidden">
						...
						<div className="absolute top-0 right-0 -mr-8 -mt-8 text-emerald-500/10 w-32 h-32">
