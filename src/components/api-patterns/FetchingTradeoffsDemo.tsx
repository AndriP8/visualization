						...
						>
							...
							<div className="grid grid-cols-3 gap-2">
								<div className="bg-surface-primary border border-border-secondary rounded-lg p-3">
									<div className="text-xs text-text-muted mb-1">Used</div>
									<div className="text-xl font-bold text-accent-green-soft">
										{usedBytes}KB
									</div>
								</div>
								<div className="bg-surface-primary border border-border-secondary rounded-lg p-3">
									<div className="text-xs text-text-muted mb-1">Wasted</div>
									<div className="text-xl font-bold text-accent-rose-soft">
										{wastedBytes}KB
									</div>
								</div>
								<div className="bg-surface-primary border border-border-secondary rounded-lg p-3">
									...
									{restFields.map((field, i) => (
										...
										>
											...
											<div className="flex items-center gap-2">
												<span className="text-text-muted">{field.bytes}KB</span>
												{field.used ? (
...
// }`}
				...
				<div className="space-y-4">
					...
					>
						...
						<div className="text-xs text-text-muted">Request exact fields</div>
					</button>
								...
								<div className="space-y-1">
									{graphqlFields.map((field, i) => (
										<motion.div
											...
											animate={{ opacity: 1, x: 0 }}
											transition={{ delay: i * 0.03 }}
											className="flex items-center justify-between px-3 py-2 rounded text-xs font-mono bg-green-500/10 text-accent-green border border-green-500/20"
										>
											<span>{field.name}</span>
											<div className="flex items-center gap-2">
												<span className="text-text-muted">{field.bytes}KB</span>
			...
			<div className="bg-surface-secondary/30 border border-border-secondary rounded-lg p-5">
				...
				<div className="space-y-2 text-sm text-text-tertiary">
					<div className="flex gap-2">
						...
						<span>
							<strong className="text-text-primary">REST over-fetches</strong> -
							wastes bandwidth but future-proof (new UI needs might already be
							satisfied)
						</span>
					</div>
					<div className="flex gap-2">
						...
						<span>
							<strong className="text-text-primary">GraphQL optimal</strong> -
							zero waste but requires query updates for new fields
						</span>
					</div>
					<div className="flex gap-2">
						<span className="text-accent-violet">•</span>
						<span>
							<strong className="text-text-primary">Trade-off</strong> - REST:
