...
export function IceNatDemo() {
	...
	const start = useCallback(async () => {
		...
		const handleCandidate =
			(peer: "A" | "B", target: RTCPeerConnection) =>
			(event: RTCPeerConnectionIceEvent) => {
				...
				target.addIceCandidate(c).catch(() => undefined);
			};

		pcA.onicecandidate = handleCandidate("A", pcB);
		pcB.onicecandidate = handleCandidate("B", pcA);
		...
		pcA.addEventListener("connectionstatechange", async () => {
			if (pcA.connectionState !== "connected") return;
	...
	return (
		<div className="space-y-6">
			...
			<div className="bg-zinc-900/60 border border-zinc-800 rounded-lg p-4 space-y-3">
				<h5 className="text-xs font-semibold text-zinc-300">Candidate types</h5>
				...
				<p className="text-[11px] text-amber-300/80 pt-1">
					...
					<code>srflx</code> (your public IP from STUN) and, when symmetric NATs
					block direct connection, <code>relay</code> (forwarded through TURN —
					bandwidth comes out of your pocket).
				</p>
			</div>
