			...
			<div className="space-y-6">
				...
				<div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-lg text-sm text-zinc-400">
					<p className="mb-2">
						...
						attributes on resources to see how browsers prioritize them.
					...
					<ul className="list-disc list-inside space-y-1 ml-2 text-xs">
						...
						<li>
							<span className="text-cyan-300">Low priority</span>: Downloaded
									...
									<div className="flex flex-wrap gap-2">
										...
										{resource.type === "image" && (
											<button
												type="button"
												onClick={() =>
													toggleAttribute(resource.id, "inViewport")
												}
												className={`px-2 py-1 text-xs rounded ${
								...
								(priority) => (
									...
									>
										<div className="flex items-center justify-between mb-2">
											<span
												className={`text-sm font-medium ${getPriorityTextColor(priority)}`}
											>
												{priority}
											</span>
											<span className="text-xs text-zinc-500">
				...
				{showWaterfall && (
					...
					>
						...
						<div className="mt-4 flex items-center gap-4 text-xs text-zinc-500">
							...
							{protocol === "http1" && (
								<span>Max 6 concurrent connections</span>
							)}
						</div>
					</motion.div>
				)}
